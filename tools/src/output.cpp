...
void printfMetadata() {
	...
	cout << "//Max Stack Size: " << _max_stack_size << "\n";
...
char * operandToString(instruction * inst, operand * op) {
	...
	switch (op->type) {
		...
		default:
			cerr << "SANITY CHECK ERROR ~1871: operand is of non-operand type.\n";
	}
	return answer;
}
...
void fprintfInstruction(ostream & out, instruction * inst) {
	...
	if(_arch < 30) {
		...
		out << " //SCHI: 0x" << std::hex << inst->SCHIVal << std::dec;
	} else if(_arch >= 50 && _arch < 70) {
		out << " SCHI50: 0x" << std::hex << inst->SCHIVal << ", " << std::dec << inst->depBarrierW << ", " << inst->depBarrierR << ", ";
		for(int x = 0; x < 6; x++) {
			if(inst->depBarrierMask & (1 << x)) {
				out << "1";
			} else {
				out << "0";
			}
		...
		cerr << "ERROR UNIMPLEMENTED o~507: don't know how to output scheduling metadata\n";
	}
}
...
void printfBlocks(bool metadata) {
	...
	node * iter2;
	while(iter) {
		blockNode * currentBlock = (blockNode*) iter->value;
		
		//Print label for block:
		printf("label");
		printf("%s", ((instruction*)currentBlock->instructions->value)->label);
		printf(":");
		
		//if(currentBlock->canLoop) {
		//	printf("\tCAN LOOP");
		//}
		
		//printf instructions inside the basic block:
		iter2 = currentBlock->instructions;
		node * iter3;
		while(iter2) {
			printf("\n\t");
			...
			//printf opcode + mods
			printf("%s", opcodes[inst->op]);
			length += strlen(opcodes[inst->op]);
			iter3 = inst->mods;
			while(iter3 != 0) {
				printf(".%s", (char*)iter3->value);
				length += 1 + strlen((char*)iter3->value);
				iter3 = iter3->next;
			}
			...
			for(x = 0; x < inst->num_operands; x++) {
				...
				iter3 = inst->operands[x]->mods;
				while(iter3 != 0) {
					//if(strcmp((char*)iter3->value, "reuse")) {
						printf(".%s", (char*)iter3->value);
						length += 1 + strlen((char*)iter3->value);
					//}
					iter3 = iter3->next;
			...
			if(_arch < 30) {
				...
				for(int x = length; x < maxLength; x++) {
					printf(" ");
				}
				printf("//SCHI: 0x%" LLx, (unsigned long long) inst->SCHIVal);
			} else if(_arch < 70) {
				for(int x = length; x < maxLength; x++) {
					printf(" ");
				}
				printf(" SCHI50: 0x%x, %d, %d, " , inst->SCHIVal, inst->depBarrierW, inst->depBarrierR);
				for(int x = 0; x < 6; x++) {
					if(inst->depBarrierMask & (1 << x)) {
						printf("1");
					} else {
				...
				cerr << "ERROR UNIMPLEMENTED o~798: don't know how to handle scheduling metadata on this architecture\n";
			}
...
void printfHex() {
	...
	while(iter) {
		...
		while(iter2) {
			...
			if(inst->guard) {
				printf("@");
				printf("%s", operandToString(inst, inst->guard));
				printf(" ");
			}
			...
			int x;
			for(x = 0; x < inst->num_operands; x++) {
				if(x > 0 && inst->op != opcode_BRX) {
					printf(",");
				}
				printf(" ");
				printf("%s", operandToString(inst, inst->operands[x]));
	...
	while(iter) {
		...
		while(iter2) {
			...
			if(inst->num_operands == 0) {
				if(_cuobjdump_version > 55 && (inst->op == opcode_EXIT || inst->op == opcode_RET || inst->op == opcode_BRK || inst->op == opcode_CONT || inst->op == opcode_SYNC)) {
					/* empty */
				} else if(_cuobjdump_version > 50 && !inst->mods && inst->op != opcode_NOP) {
					printf(" ");
				}
			}
