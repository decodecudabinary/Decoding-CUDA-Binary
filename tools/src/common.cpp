...
void initBlock(blockNode** block) {
	*block = (blockNode*) malloc(sizeof(blockNode));
	...
	(*block)->last = 65535;
	(*block)->size = 0;
	(*block)->end = end_generic;
	(*block)->guarded = false;
	(*block)->force_guard = false;
	(*block)->tag = 0;
	(*block)->target = 0;
...
instruction* newInstruction(opcode op, node * mods, operand * o1, operand * o2, operand * o3, operand * o4, operand * o5) {
	instruction * inst = newInstruction(op, mods);
	inst->num_operands = 5;
	inst->operands = (operand**) malloc(5 * sizeof(operand*));
	inst->operands[0] = o1;
	inst->operands[1] = o2;
	inst->operands[2] = o3;
	inst->operands[3] = o4;
...
node * stringsToList(const char * str1, const char * str2, const char * str3) {
	node * list = 0;
...
long long instructionorder(void * a, void * b) {
	instruction * s = (instruction*) a;
	instruction * t = (instruction*) b;
	return s->line - t->line;
}

long long blockorder(void * a, void * b) {
	blockNode * s = (blockNode*) a;
	blockNode * t = (blockNode*) b;
...
void cleanBlock(blockNode* block) {
	node * n = block->instructions;
	node * next;
	while(n != 0) {
		next = n->next;
		instruction * i = (instruction*) n->value;
		cleanInstruction(i);
		free(n);
		n = next;
	}
	cleanNodes(block->forward);
	cleanNodes(block->branches);
	cleanNodes(block->predecessors);
